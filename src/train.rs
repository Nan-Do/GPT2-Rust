...
pub fn train(
    ...
    let split_idx = (text_data.len() as f32 * options.train_ratio) as usize;
    let train_data = &text_data[0..split_idx];
    let val_data = &text_data[split_idx..text_data.len()];

    let train_dataset = TokenizedDataset::new(tokenizer, train_data, options.max_seq_len);
    let val_dataset = TokenizedDataset::new(tokenizer, val_data, options.max_seq_len);
